                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto mb-16">
                    {prizes.map((prize, index) => (
                        <Card
                            ...
                            className={`border-2 ${
                                index === 0
                                    ? "border-event-gold md:-translate-y-4"
                                    : `border-${prize.color}`
                            } hover:shadow-elevated transition-all duration-300 bg-background`}
                        >
                            <CardHeader className="text-center pb-4">
                                <div
                                    className={`w-20 h-20 mx-auto mb-4 rounded-full bg-${prize.color}/10 flex items-center justify-center`}
                                >
                                    <prize.icon
                                        className={`h-10 w-10 text-${prize.color}`}
                                    />
                                </div>
                            ...
                            <CardContent className="text-center">
                                <p
                                    className={`text-4xl font-bold text-${prize.color} mb-2`}
                                >
                                    {prize.prize}
                                </p>
                ...
                <div className="max-w-4xl mx-auto">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {sideGames.map((game) => (
                            <Card
                                key={game.name}
                                className="border-accent/50 bg-background hover:shadow-elevated transition-all"
                            >
